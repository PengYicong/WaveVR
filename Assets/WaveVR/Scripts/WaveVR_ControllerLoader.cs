...
public class WaveVR_ControllerLoader : MonoBehaviour {
    ...
    private void onLoadController(WVR_DeviceType type)
    {
        ...
        if (Application.isPlaying)
        {
            ...
            {
                PrintDebugLog("Cant load generic controller model, Please check file under Resources/" + controllerModelFolder + genericControllerFileName + ".prefab is exist!");
            }
            else
            {
                ...
                Rigidbody rb = controllerPrefab.AddComponent<Rigidbody>() as Rigidbody;
                SphereCollider sc = controllerPrefab.AddComponent<SphereCollider>() as SphereCollider;
                rb.isKinematic = true;
                sc.radius = 0.15f;
                controllerPrefab.tag = "Player";
